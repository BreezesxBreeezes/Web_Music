...
const exploreRadioMoveBtns = Array.from(
  $$(".tab--explore .container__move-btn.move-btn--radio")
);
const radioTabMoveBtns = Array.from(
  $$(".tab--radio .container__move-btn.move-btn--radio")
);
...
const app = {
  ...
  normalPlaylists: JSON.parse(
    localStorage.getItem(NORMAL_PLAYLIST_STORAGE_KEY) || "[]"
  ),

  listSongCharts: JSON.parse(
    localStorage.getItem(SONG_CHARTS_STORAGE_KEY) || "[]"
  ),

  posts: JSON.parse(localStorage.getItem(POST_STORAGE_KEY) || "[]"),

  durationList: JSON.parse(
                                            ...
                                            <div class="media__info">
                                                ...
                                                <p class="info__author">
                                                    ${mv.author.map(
                                                      (author, index) => {
                                                        return app.html`
                                                            <a href="#" class="is-ghost">${author}</a>${
                                                          index <
                                                            mv.author.length -
                                                              1 && ", "
                                                        }
                ...
                ${this.exploreSlides.map((exploreSlide, index) => {
                  return app.html`
                        <div 
                            ...
                            ${
                              index > 3 &&
                              index < app.exploreSlides.length - 1 &&
                              "fifth"
                            }
                            ${
                              index === app.exploreSlides.length - 1 &&
                            ...
                            <div class="row__item item--new-playlist">
                                <div class="row__item-container flex--top-left">
                                    ...
                                    <div class="row__item-info new-playlist--info">
                                        ...
                                        <h3 class="row__info-creator">
                                            ${newPlaylist.singer.map(
                                              (singer, index) => {
                                                return app.html`
                                                    <a href="#" class="is-ghost">${singer}</a>${
                                                  index <
                                                    newPlaylist.singer.length -
                                                      1 && ","
                                                }
                                                `;
                                              }
                    ...
                    <div class="container__header special-playlist--header">
                        <div class="row__item-info media">
                            <div class="media__left">
                                <div class="row__item-display br-5">
                                    <div class="row__item-img img--square" style="background: url('${
                                      app.specialPlaylists[playlistListIndex]
                                        .header.image
                                    }') no-repeat center center / cover"></div>
                                </div>
  ...
  renderNormalPlaylist() {
    normalPlaylistLists.forEach((normalPlaylistList, playlistListIndex) => {
      normalPlaylistList.innerHTML = app.html`
                ...
                <div class="col l-12 m-12 c-12">
                    <div class="row ${
                      playlistListIndex !== 7 && "no-wrap"
                    } normal-playlist--container">
                        ${app.normalPlaylists[playlistListIndex].playlists.map(
                          (playlist, index) => {
                            return app.html`
                                <div class="col l-2-4 m-3 c-4 ${
                                  playlistListIndex === 7 && "mb-30"
                                }">
                                    <div class="row__item item--playlist">
                                        <div class="row__item-container flex--top-left">
                                            <div class="row__item-display br-5">
  ...
  handleEvents: function () {
        ...
        if (!_this.isSeeking) {
          ...
          if (
            ...
            localStorage.setItem(
              ...
              JSON.stringify(_this.durationList)
            );
            listDurationTime.innerHTML =
              _this.durationList[_this.currentPlaylist][_this.currentIndex];
    ...
    const playlistItems = $$(
      ".tab-home .playlist--container .row__item.item--playlist:not(.playlist--create)"
    );
    ...
    const tabPlaylistItems = $$(
      ".tab-playlist .playlist--container .row__item.item--playlist:not(.playlist--create)"
    );
    ...
    sidebarSubnavItems.forEach((subnavItem) => {
      subnavItem.onclick = (e) => {
        showNotificationToast(
          "Tính năng hiện chưa được cập nhật, Chúng tôi sẽ phát triển trong tương lai!"
        );
      };
    });
  ...
  loadCurrentPlaylist(playlistBtn, index) {
    if (playlistBtn) {
      if (index < 4) {
        ...
        showNotificationToast(
          "Trang web hiện tại chưa hoàn thiện, Huyền Anh vui lòng chọn 4 playlist đã được cập nhật!"
        );
      }
    }
  },
  ...
  loadCurrentSong: function () {
    ...
    authors.forEach((author) => {
      author.innerHTML = app.html`
                ${this.currentSong.singer.map((singer, index) => {
                  return app.html`<a href="#" class="is-ghost">${singer}</a>${
                    index < this.currentSong.singer.length - 1 && ", "
                  }`;
                })}
    ...
    durationTimes.forEach((durationTime) => {
      durationTime.innerHTML =
        this.durationList[this.currentPlaylist][this.currentIndex];
    });
  ...
  loadConfig: function () {
    ...
    audio.volume =
      this.config.currentVolume == 0 ? 0 : this.config.currentVolume / 100 || 1;
    if (!audio.volume) {
      volumeIcons.forEach((volumeIcon) => {
        volumeIcon.classList.replace("bi-volume-up", "bi-volume-mute");
      });
    ...
    volumeTracks.forEach((volumeTrack) => {
      volumeTrack.style.width =
        (this.config.currentVolume == 0
          ? 0
          : this.config.currentVolume || 100) + "%";
    });
    durationTimes.forEach((durationTime) => {
      durationTime.textContent = this.audioCalTime(
        this.durationList[this.currentPlaylist][this.currentIndex]
      );
    });
    randomBtns.forEach((randomBtn) => {
  ...
  setUpRender: function () {
    ...
    if (this.durationList[this.currentPlaylist].length === 0) {
      this.songs.forEach((song, index) =>
        this.durationList[this.currentPlaylist].push("--/--")
      );
    }
  },
  ...
  nextSlide: function () {
    ...
    const sixthSlide = $(".explore__slide-item.sixth");
    const fifthSlideIndex =
      slideMoveItems.indexOf(fourthSlide) === slideMoveItems.length - 1
        ? 0
        : slideMoveItems.indexOf(fourthSlide) + 1;
    const fifthSlide = slideMoveItems[fifthSlideIndex];
  ...
  playRandomSong: function () {
    ...
    do {
      ...
      this.indexArray.includes(newIndex)
    );
